...
<head>
    ...
    <style>
        ...
        .sidebar-link.active { @apply bg-white text-orange-600; }
    </style>
</head>
...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
       class="fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-orange-500 to-orange-700 text-white transition-transform duration-300 flex flex-col shadow-xl">

    {{-- Logo --}}
    <div class="flex items-center gap-3 px-6 py-5 border-b border-white/20">
        <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center">
            <span class="text-orange-600 font-extrabold text-sm">SI</span>
        </div>
        <div>
            <div class="font-bold text-lg leading-none">SI-IMUT</div>
            <div class="text-xs text-orange-200">Sistem Informasi Mutasi</div>
        </div>
    </div>
    ...
    <div class="px-4 py-4 border-t border-white/20">
        <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="text-xs text-orange-200 capitalize">{{ auth()->user()->role }}</div>
            </div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="text-orange-200 hover:text-white transition" title="Logout">
...
<div :class="sidebarOpen ? 'ml-64' : 'ml-0'" class="transition-all duration-300 min-h-screen flex flex-col">
    ...
    <header class="sticky top-0 z-30 bg-white shadow-sm flex items-center justify-between px-6 py-3">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="w-9 h-9 bg-orange-500 rounded-full flex items-center justify-center text-white text-sm font-bold">
                {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
            </div>
        </div>
    </header>
